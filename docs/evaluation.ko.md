...
| M0 config CLI | 4-validator config 생성 | `cargo run -- init-config` | JSON config 출력 |
| M0 payload CLI | example decision payload prepare | `cargo run -- prepare --config /tmp/commonware-bft-lab.json --payload examples/decision-payload.json` | payload hash 출력 |
| docs navigation | docs index, Mermaid flow, milestone checklist | `docs/README.ko.md`와 `docs/milestones.ko.md` 확인 | 존재함 |
...
## 다음 Acceptance Targets

| 다음 마일스톤 | 구체적 목표 | 필수 테스트 |
|---|---|---|
| M1 simulator | delay/drop rule을 가진 deterministic scheduler | `sim_replays_same_transcript_for_same_seed`, `sim_delays_messages_by_rule`, `sim_drops_messages_by_rule` |
| M2 PBFT | 4-validator PBFT baseline이 block 1개를 commit하고 equivocation을 거부 | `pbft_commits_with_four_validators_and_no_faults`, `pbft_never_commits_two_blocks_at_same_height` |
| M5 Commonware devnet | local validator 4개가 `DecisionPayload` 1개를 finalize | `commonware_payload_hash_matches_prepared_block`, `decision_log_appends_finalized_payload_once` |
